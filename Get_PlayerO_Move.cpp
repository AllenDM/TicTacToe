#include "tttBoard.h"
//-| --------------------------------------------------------------
//-| Get move for player O: row and column positions.
//-| --------------------------------------------------------------
void Get_PlayerO_Move(tttBoard & B, int & row, int & col)
{
...
//-|Major Diagonal opponent mark counter
   ...
   if(MajorDcount == (B.Size()-1))
 {
  ...
  //-| Places mark in Minor Diagonal Configuration if unblocked
   if(MinorDcount < 1)
{ 
   c = B.Size();
   for (r=1;  r<=B.Size(); r++)
...
//-|Places Mark in Major Diagonal Configuration if unblocked
     c = B.Size();
if(MajorDcount < 1)
{
for (r=B.Size();r>=1;r--)
